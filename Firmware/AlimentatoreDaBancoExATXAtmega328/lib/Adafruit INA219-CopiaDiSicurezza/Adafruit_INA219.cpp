...
/*!
 *  @brief  Configures to INA219 to be able to measure with user parameter
 *  @brief  Sets up the HW with the global parameter 
            DefaultKCorrectionV
            DefaultVolt, 
            DefgaultAmp,
            DefaultRShunt
 ...
 *  @note   These calculations assume a 0.1 ohm resistor is present
 */
bool Adafruit_INA219::setCalibration_vvV_aaA
                      (float udRShunt, uint16_t udVShunt, 
                       float udCurrentMaxA, float udCurrentLSBmA, 
                       uint8_t udVMax,
                       uint16_t BADCRes , uint16_t SADCSamp) 
{
  if((udCurrentMaxA> 0) && 
     (udCurrentLSBmA >  0) &&
     (udVMax > 0) &&
     ((udCurrentMaxA*1000) > udCurrentLSBmA) && 
     (udRShunt > 0) &&
     (udVShunt > 0))
  {
    ...
    // RSHUNT = 0.00265			(Resistor value in ohms)
    ...
    // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
    // MinimumLSB = MaxExpected_I/32767
    // MinimumLSB = 0.00091            (910 uA per bit)
    // MaximumLSB = MaxExpected_I/4096
    // MaximumLSB = 0.00732              (7,3 mA per bit)
  
    // 4. Choose an LSB between the min and max values
    //    (Preferrably a roundish number close to MinLSB)
    // CurrentLSB = 0.001 (1 mA per bit)
    // 5. Compute the calibration register
    //////// Cal = 10240 (0x2800)
    // Cal = 15456 
    ina219_calValue = (0.04096 / ((udCurrentLSBmA/1000) * udRShunt));
  
    // 6. Calculate the power LSB
    // PowerLSB = 20 * CurrentLSB
    // PowerLSB = 0.020 (20mW per bit)
    ...
    // Set multipliers to convert raw current/power values
    //////////ina219_currentDivider_mA = 25;    // Current LSB = 40uA per bit (1000/40 = 25)
    ina219_currentDivider_mA = udCurrentLSBmA;    // Current LSB = 1mA per bit 
    ina219_powerMultiplier_mW = 20 * udCurrentLSBmA; // Power LSB = 20mW per bit
    ...
    if (udVMax<17)
    {
      Vmax = INA219_CONFIG_BVOLTAGERANGE_32V;
    }
    else
    {
      Vmax = INA219_CONFIG_BVOLTAGERANGE_16V;
    }
    ...
    case 40:
      ...
      break;
    case 80:
      Gain = INA219_CONFIG_GAIN_2_80MV;
      break;
    case 160:
      Gain = INA219_CONFIG_GAIN_4_160MV;
      break;    
    default:
